...
static auto runAllTests() -> int {
  return testcpplite::test(
      {{parses::zeroDollars, "parses zero (0) as USD"},
       {parses::oneDollar, "parses one (1) as USD"},
       {parses::oneDollarTwentyThreeCents,
        "parses number having two decimal places (X.XX) as USD"},
       {parses::oneDollarTwentyCentsWithoutTrailingZero,
        "parses number having one decimal place (X.X) as USD"},
       {parses::oneCent, "parses one one-hundredth (0.01) as USD"},
       {parses::tenCents, "parses one-tenth (0.1) as USD"},
       {parses::twelveCentsWithoutLeadingZero,
        "parses number without leading zero (.X) as USD"},
       ...
       {stream::toAccounts2, "stream loads accounts 2"},
