...
static auto runAllTests() -> int {
  return testcpplite::test(
      {{parse::zeroDollars, "parse zero (0) as USD"},
       {parse::oneDollar, "parse one (1) as USD"},
       {parse::oneDollarTwentyThreeCents,
        "parse number having two decimal places (X.XX) as USD"},
       {parse::oneDollarTwentyCentsWithoutTrailingZero,
        "parse number having one decimal place (X.X) as USD"},
       {parse::oneCent, "parse one one-hundredth (0.01) as USD"},
       {parse::tenCents, "parse one-tenth (0.1) as USD"},
       {parse::twelveCentsWithoutLeadingZero,
        "parse number without leading zero (.X) as USD"},
       {parse::twelveCentsIgnoringThirdDecimalPlace,
        "parse number having three decimal places (X.XXX) as USD"},
       {parse::oneCentIgnoringThirdDecimalPlace,
