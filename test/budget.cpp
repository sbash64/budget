...
static void debit(BudgetInMemory &budget, std::string_view accountName,
                  const Transaction &t) {
  budget.debit(accountName, t);
}

static void credit(BudgetInMemory &budget, const Transaction &t = {}) {
  budget.credit(t);
}
